...
final class Selection {
	...
	public Selection getExtendedSelection(Selection other) {
		...
		if ((selectionType == SelectionType.CELLS && this.type != other.type)
			...
			// TODO I think it would be a lot easier to understand this operation by splitting
			//  the if up into different cases. I can't wrap my head around what the if statement
			//  actually means. Also, maybe a comment would be helpful.
		...
		// TODO The above case takes the min/max of rows/coumns, but this case here does not need
		//  to take mix/max into account? The resulting row and column ranges can be a mix
		//  of positive and negative values? Maybe a comment would be helpful.
		return new Selection(selectionType, TabularRange.range(
